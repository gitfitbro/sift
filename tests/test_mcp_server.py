...
class TestSiftBuildOutputs:
    def test_build_no_data(self, sample_session):
        ...
        assert result["status"] == "error"
        assert (
            "extracted data" in result["error"].lower() or "extraction" in result["error"].lower()
        )
...
class TestSiftAnalyzeProject:
    def test_analyze_project(self, tmp_path):
        ...
        assert (
            "python" in str(result["project"]["languages"]).lower()
            or "py" in str(result["project"]["languages"]).lower()
        )
