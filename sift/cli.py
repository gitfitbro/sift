...
from sift.ui import banner, console
...
def main_callback(
    ...
    from sift.ui import set_json_mode, set_plain_mode
...
def run(
    ...
    phase: str = typer.Option(
        None, "--phase", "-p", help="Start at specific phase", autocompletion=complete_phase_id
    ),
    no_tui: bool = typer.Option(
        False, "--no-tui", help="Force Rich fallback instead of Textual TUI"
    ),
):
    ...
    if not no_tui:
        try:
            ...
            tui = SiftApp(session, start_phase=phase)
            tui.run()
            return
        except ImportError:
            pass
...
def open_session(
    ...
    if not no_tui:
        try:
            ...
            tui = SiftApp(session, mode="workspace")
            tui.run()
            return
        except ImportError:
            pass
...
def analyze(
    ...
    template: bool = typer.Option(
        False, "--template", "-t", help="Generate a session template recommendation"
    ),
    ...
    if structure.directory_tree:
        console.print(
            Panel(structure.directory_tree, title="Directory Structure", border_style="dim")
        )
    ...
    if top_complex:
        ...
        for fa in top_complex:
            try:
                ...
                rel = str(fa.path)
            cx_table.add_row(
                rel, str(fa.line_count), str(len(fa.functions)), f"{fa.complexity_score:.1f}"
            )
    ...
    if template:
        ...
        for i, phase in enumerate(rec.phases, 1):
            phase_table.add_row(
                ...
                (phase.get("prompt", "")[:80] + "...")
                if len(phase.get("prompt", "")) > 80
                else phase.get("prompt", ""),
            )
...
def models(
        ...
        try:
            ...
            if server_models:
                for m in server_models:
                    ...
                    active = (
                        "*" if active_provider == "ollama" and model_name == active_model else ""
                    )
    ...
    console.print()
    console.print(
        "[dim]Switch provider:[/dim]  sift --provider ollama --model llama3.2 run my-session"
    )
