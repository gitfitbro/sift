...
class SessionRunnerScreen(Screen):
    ...
    def _refresh_ui(self) -> None:
        ...
        current_pt = self._phases[self._current_phase_idx] if self._current_phase_idx < len(self._phases) else None
        ...
        if status == "pending":
            ...
            all_done = all(
                self._session.phases.get(p.id) and self._session.phases[p.id].status in ("extracted", "complete")
    ...
    def handle_capture(self, event: CaptureForm.Submitted) -> None:
        ...
        if event.mode == "text":
            self._do_capture_text(current_pt.id, event.content)
        elif event.mode == "file":
            self._do_capture_file(current_pt.id, event.content)

    @work(thread=True)
    def _do_capture_text(self, phase_id: str, text: str) -> None:
        """Capture text in a worker thread."""
        try:
            result = self._extraction_svc.capture_text(self.session_name, phase_id, text)
            self.app.call_from_thread(
                self.notify, f"Text captured ({result.char_count} chars)", severity="information"
            )
    ...
    @work(thread=True)
    def _do_capture_file(self, phase_id: str, file_path: str) -> None:
        """Capture file in a worker thread."""
        try:
            result = self._extraction_svc.capture_file(
                self.session_name, phase_id, Path(file_path)
            )
            self.app.call_from_thread(
                self.notify, f"File captured ({result.file_type})", severity="information"
            )
        except Exception as e:
            self.app.call_from_thread(self.notify, str(e), severity="error")
        self.app.call_from_thread(self._refresh_ui)
